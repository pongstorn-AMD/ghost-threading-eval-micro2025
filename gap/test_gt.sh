...
#----------only set these parameters----------
# Usage: ./test_gt.sh <kernel> <graph> <smt_core0> <smt_core1> <out_path> [read] [baseline]
kernel_name=$1
graph_name=$2
smt_core0=$3
smt_core1=$4
...
# run_mode=baseline -> only baseline (skip swpf / htpf / best)
...
if [ "$exe_baslines" == "1" ]; then
	...
	if [ "$read_only" == "read" ]; then
		perf stat -a -euncore_imc_*/cas_count_read/,uncore_imc_*/cas_count_write/ taskset -c $smt_core0 ./$kernel_name -f $graph -x -n $repeat > $out_pf 2>&1
	else
...
if [ "$exe_swpf" == "1" ]; then
	...
	echo "SWPF: $out_pf."
	if [ "$read_only" == "read" ]; then
		perf stat -a -euncore_imc_*/cas_count_read/,uncore_imc_*/cas_count_write/ taskset -c $smt_core0 ./$kernel_name-swpf -f $graph -x -n $repeat > $out_pf 2>&1
	else
		...
		do
			...
			do
				...
				if [ $un_threshold -le $threshold ]; then
					...
					echo "HTPF: $out_pf."
					if [ "$read_only" == "read" ]; then
						perf stat -a -euncore_imc_*/cas_count_read/,uncore_imc_*/cas_count_write/ taskset -c $smt_core0,$smt_core1 ./$kernel_tpf_name -f $graph -x -n $repeat -p $syncfreq -o $threshold -j $skip -q $un_threshold > $out_pf 2>&1
					else
